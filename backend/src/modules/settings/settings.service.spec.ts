...
describe('SettingsService', () => {
  ...
  beforeEach(async () => {
    ...
    const revisions: CvRevision[] = [];
    cvRepository = {
      findOne: jest.fn(
        async ({ where }: any) =>
          revisions.find((item) => item.id === where.id) ?? null,
      ),
      find: jest.fn(async () =>
        [...revisions].sort((a, b) => b.revision - a.revision),
      ),
      create: jest.fn((value: Partial<CvRevision>) => value as CvRevision),
      save: jest.fn(async (value: CvRevision) => {
        const saved = {
          ...value,
          id: value.id ?? `cv-${value.revision}`,
        ...
        async (callback: (manager: any) => Promise<void>) => {
          ...
          const result = await callback({
            getRepository: (entity: unknown) =>
              entity === CvRevision ? cvRepository : transactionalRepository,
          });
          values = pending;
          repository.findOne.mockImplementation(({ where }: any) =>
            Promise.resolve(
              values.has(where.key)
    ...
    const module = await Test.createTestingModule({
      providers: [
        SettingsService,
        { provide: getRepositoryToken(Setting), useValue: repository },
        { provide: getRepositoryToken(CvRevision), useValue: cvRepository },
      ],
    }).compile();
  ...
  it('migrates legacy CV content exactly once', async () => {
    ...
    expect(cv).toMatchObject({
      content: '  Exact legacy text\n',
      source: 'legacy_url',
      revision: 1,
    });
    expect((values.get('master_cv_state') as any).current.updated_at).toBe(
      '2025-01-02T03:04:05.000Z',
    );
  });

  it('saves, clears, and swaps the one-step backup for undo and redo', async () => {
    await service.saveMasterCv({
      content: 'First version',
      source: 'manual',
      expected_revision: 0,
    });
    const second = await service.saveMasterCv({
  ...
  it('does not create a revision or backup for identical content', async () => {
    ...
    const result = await service.saveMasterCv({
      content: 'Same',
      source: 'manual',
      expected_revision: 1,
    });
  ...
  it('stores the complete CV while exposing only text outside exact exclusion markers to AI', async () => {
    const content =
      'Public\n<!-- AI-EXCLUDE-START -->private<!-- AI-EXCLUDE-END -->\nSkills';
    await service.saveMasterCv({
      content,
      source: 'manual',
      expected_revision: 0,
    });
    expect((await service.getMasterCv()).content).toBe(content);
    expect((await service.getMasterCvContext()).text).toBe('Public\n\nSkills');
    await expect(
      service.saveMasterCv({
        content: '<!-- AI-EXCLUDE-START -->private',
        source: 'manual',
        expected_revision: 1,
      }),
  ...
  it.each([
    ...
    await expect(service.saveMasterCv(input)).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });
  ...
  it('validates general updates before starting a transaction', async () => {
    await expect(
      service.updateSettings({ score_threshold: 101 }),
    ).rejects.toBeInstanceOf(BadRequestException);
  ...
  it('reports URL refresh failures without mutating the CV', async () => {
    ...
    await expect(service.refreshCv()).rejects.toBeInstanceOf(
      InternalServerErrorException,
    );
