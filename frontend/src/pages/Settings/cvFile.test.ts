...
describe('decodeCvFile', () => {
  it('accepts case-insensitive text extensions and strips a UTF-8 BOM', async () => {
    const file = new File([new Uint8Array([0xef, 0xbb, 0xbf]), '# CV'], 'Resume.MD');
    await expect(decodeCvFile(file)).resolves.toEqual({ content: '# CV', filename: 'Resume.MD' });
  });
  ...
  it.each([
    ...
    [new File([new Uint8Array([0xc3, 0x28])], 'cv.txt'), 'The file is not valid UTF-8.'],
    [new File([new Uint8Array(MASTER_CV_MAX_BYTES + 1)], 'cv.txt'), 'The file must be 1 MiB or smaller.'],
